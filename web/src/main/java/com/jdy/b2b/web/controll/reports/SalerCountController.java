...
public class SalerCountController extends BaseController{
    ...
    @ApiOperation("列表查询")
    @PostMapping("list")
    public ResultBean<Map<String,Object>> querySalerCountPage(@RequestBody @Validated SalerCountQueryVo vo){
        return salerCountService.querySalerCountPage(vo);
    }

    @ApiOperation("导出报表,不分页")
    @GetMapping("export/{str}")
    //public ResultBean exportSalerCount(@RequestBody @Validated SalerCountQueryVo vo, HttpServletResponse response) throws IOException {
    public ResultBean exportSalerCount(@PathVariable String str , HttpServletResponse response) throws Exception {
        SalerCountQueryVo vo = new SalerCountQueryVo();
        vo.setSearchStr(str);
        ResultBean<Map<String,Object>> resultBean = salerCountService.querySalerCountPage(vo);
        Map<String,Object> map = resultBean.getParsedEnitity(Map.class);
        List result = (List<SalerCountDO>)map.get("list");
        List<SalerCountDO> list = JSONUtil.trans2List(JSON.toJSONString(result), SalerCountDO.class);
        exportExcel(list,response);
        return ResultBean.getSuccessResult();
    }

    private void exportExcel(List<SalerCountDO> list, HttpServletResponse response) throws IOException {
        //1.创建画布
        SXSSFWorkbook wb = new SXSSFWorkbook();
        SXSSFSheet sheet = wb.createSheet("供应商商汇总");
        //准备表头数据
        ...
        if(list!=null && list.size()>0) {
            for (int m = 0; m < list.size(); m++) {
                ...
                SalerCountDO rowData = (SalerCountDO) list.get(m);
                // 第四步，创建单元格，并设置值
                ...
                cell6.setCellValue(rowData.getBuyerCounts() == null ? 0 : rowData.getBuyerCounts());
        ...
        try {
            ...
            String fileName = "供应商汇总报表" + dateStr;
