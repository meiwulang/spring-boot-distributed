    ...
    <div class="jdy-content-inner">
      <el-form :model="banqiForm" :rules="banqiRule" ref="banqiForm" class="demo-form-inline" label-width="100px">
        <div class="jdy-tab border">
          <el-button @click="goback('ProductDeployInventory')" class="fright mt10 mr10">返回</el-button>
            ...
            <div class="p10 border pb30 schedule-one">
              ...
              <el-table ref="multipleTable" :data="ticketList" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="" label="库存量（-1代表无限制）" width="">
                  <template scope="scope">
                    <el-input-number v-model="scope.row.tStock" :controls="false" :min="-1" :max="1000000"></el-input-number>
                  </template>
                </el-table-column>
              </el-table>
            </div>
        ...
        if (valid) {
          ...
          this.$http.post(this.saveUrl, this.banqiForm).then(response => {
            if(response.data.code==0){
              ...
              setTimeout(() => {
                this.btnFlag = false;
                this.$router.push({ name: "ProductDeployInventory", query: { id: this.$route.query.id,c_id:this.$route.query.c_id } });
              }, 1000)
            }else{
              this.btnFlag = false;
