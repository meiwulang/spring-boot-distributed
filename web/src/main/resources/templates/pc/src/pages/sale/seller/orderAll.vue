...
<div class="jl-information">
    <div class="jdy-content jdy-transfer yjk-fleft noborder">
        <div class="jdy-tab" style="margin:0;padding-right:0">
            ...
            <a href="javascript:;" title="" @click="jdytab(11)" :class="{ active: jdytabActive==11 }">待确认&nbsp;&nbsp;<div class="yjk-number">{{orderTotal.statuses_11}}</div>
                <span></span>
            </a>
            ...
            <el-form :model="ruleform" :inline="true" ref="ruleform" class="demo-form" label-width="150px">
                <el-row class="mt10">
                    ...
                    <el-form-item class="yjk-mb10 yjk-search">
                        <el-input v-model="ruleform.searchKey" placeholder="请输入产品名称、订单号、游客姓名、手机号搜索"></el-input>
                    </el-form-item>
                    <el-form-item class="yjk-mb10" :class="{ paddLeft: logindata.uDataLimit!=3 }">
            ...
            <el-table :data="tableData" border class="mt20 all yjk-orderList" highlight-current-row @current-change="handleRowChange">
                            ...
                            <template scope="scope" >
                                ...
                                <div class="ptlr20 textCenter">
                                    ...
                                    <el-select  @change="orderChange" placeholder="订单修改" value="" v-model="orderChangeType[scope.row.id]" class="yjk-orderChangeSel"  v-show="scope.row.alterTicketId!=null">
                                      ...
                                      <el-option key="10" label="改签确认" value="10" v-show="showEndorse== true && scope.row.lastAlterTicketStatus==0"></el-option>
                                    </el-select>
                ...
                <el-table-column label="订单操作" min-width="250">
                    <template scope="scope" >
                        ...
                        <div class="ptlr20 textCenter">
                            <el-button type="primary" @click="orderDetailGoto(scope.row)" class="yjk-orderDetailBtn">订单详情</el-button>
                            <!-- <el-button @click="checkContractAlert(scope.row.id)" class="yjk-orderChangeSel">审核合同</el-button> v-show="scope.row.oStatus!=4"-->
                            <el-select  @change="orderChange" placeholder="订单修改" value="" v-model="orderChangeType[scope.row.id]" class="yjk-orderChangeSel"  :disabled="scope.row.oStatus==4">
                                ...
                                <el-option key="9" label="退款确认" value="9" v-show="scope.row.refundRecordStatus != null && scope.row.refundRecordStatus == 1"></el-option>      
                                <el-option key="10" label="改签确认" value="10" v-show="showEndorse== true && scope.row.lastAlterTicketStatus==0"></el-option>
                                <!-- <el-option key="11" label="关闭改签" value="11" v-show="scope.row.lastAlterTicketStatus==0"></el-option> -->
                            </el-select>
                            <!-- <el-button type="primary" @click="quickPay(scope.row.id)" class="yjk-orderDetailBtn" v-show="scope.row.oStatus==0">立即支付</el-button>                             -->
                        </div>
                    </template>
    ...
    <jdy-alert title="改签确认" @closeAlert="closeAlert" v-if="changeSignFlag" class="alertCityList showtfcity refund" style="width:800px;">
      ...
      <div class="signlist signConfirmTouristsList" style="padding-top:15px;">
          <div class="signConfirmList" v-for="item in signConfirmList" :key="item">
            <span>{{item.touristName}}-{{item.touristLincese}}-{{item.ticketName}}</span>
          </div>
      </div>
      <div class="touristHead">
      ...
      <table border="1" class="signlist" style="height:52px;">
        <tr>
          ...
          <td class="borderR borderB">团期编号：{{signConfirmData.scheduleNo}}</td>
      ...
      <div class="alertfoot1 clearfix">
        ...
        <el-button class="fright mt10 mr10" @click="closeSignFun">取消改签申请</el-button>
        <el-button class="fright mt10" @click="saveSignForm" type="primary">确认改签申请</el-button>
      </div> 
    </jdy-alert>
    <!--改签确认 弹窗  end-->
     <!--退款记录 弹窗  begin-->
    ...
    <!--退款确认 弹窗  end-->
    <!--上传支付凭证 弹窗  begin-->
    <jdy-alert title="确认收款：" @closeAlert="closeAlert" v-if="payedFlag" class="alertCityList showtfcity" style="width:1400px">
      <el-table :data="getVouchers" style="width: 100%"  border max-height="500" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="上传说明" prop="uploadDesc" min-width="120" class-name="red-font">
          <template scope="scope">
        ...
        <el-table-column label="实收时间" min-width="200">
          <template scope="scope">
            <el-date-picker v-model="getVouchers[scope.$index].uploadTime " type="datetime" placeholder="选择日期时间">
            </el-date-picker>
          </template>
        </el-table-column>        
        <el-table-column label="操作" min-width="80">
          <template scope="scope">
              <el-button type="default" size="mini" @click.native.prevent="voucherError(scope.row,scope.$index)" v-if="scope.row.fStatus == 0">
                  凭证错误
              </el-button>   
          </template>
        </el-table-column>
      ...
      <div class="alertfoot1 clearfix">
        <el-button class="fright mt10 mr10" @click="closeAlert">关闭</el-button>
        <el-button class="fright mt10 mr10" @click="savePay" v-show="showSave">确认</el-button>
      </div> 
    </jdy-alert>
    <!--上传支付凭证 弹窗  end-->
    <!--调整金额 弹窗  begin-->
...
export default {
  ...
  mounted() {
    setTimeout(()=>{
      ...
      if(this.menuData.indexOf("confirmEndorse") != -1){
  ...
  methods: {
          ...
          for (let i = 0; i < data.extObj.length; i++) {
            switch (data.extObj[i].status) {
              ...
              case 10:
                ...
                break;
              case 11:
                this.orderTotal.statuses_11 = data.extObj[i].number;
                break;
            }
          }
    ...
    jdytab(index) {
      ...
      //   this.ruleform.dateEnd=null;
      //   this.datePicker=null
      // }else{       
      //   this.ruleform.dateStart=dateChange(-30); 
      //   this.ruleform.dateEnd=dateChange(0);        
      //   this.datePicker=[
    ...
    orderDetailGoto(order) {
      ...
      this.$router.push({ name: "orderSellerDetail", query: { id: order.id,type:type } });
    },
    // 调整金额
    priceChange_submit() {
    ...
    // 获取改签相关信息
    getSignInfo(id){
      this.$http.get(api_prefix+"alterTicket/beforeAlterTicketRecord/"+id).then(response=>{
        if (response.body.code == 0) {
          this.signConfirmList=response.body.body.touristInfo;
          this.signConfirmData.signStartDate=response.body.body.newStartDate
          this.signConfirmData.scheduleNo=response.body.body.targetGroupNo   
        } else {
    ...
    changeSignFun(id){
      ...
      this.getSignInfo(this.rowData.lastAlterTicketId)
    },
    saveSignForm(){
      let param={
        alterId:this.rowData.lastAlterTicketId,
        status:1
      }
      this.$http.post(api_prefix + "alterTicket/changeTicket/",param).then(response => {
        if (response.body.code == 0) {
          this.closeAlert();
    ...
    closeSignFun(id){
      ...
      this.$confirm("确定关闭改签？为避免纠纷，请确保游客已同意关闭改签", "温馨提示", {
        ...
        .then(() => {
          this.$http.post(api_prefix + "alterTicket/changeTicket",param).then(
            response => {
      ...
      this.$http.post(api_prefix + "/Order/reviewVoucher", a).then(
        response => {
          if (response.data.code == 0) {
            ...
            // 从列表里面删除
            setTimeout(()=>{
              this.getVouchers.splice(index, 1);
            },300)
          } else {
            this.$alert(response.data.message);
          }
    ...
    savePay(){
      if(this.multipleSelection.length==0){
        return this.$message.info("请选择需要确认的凭证记录！")
      }
        this.$confirm('确认所有凭证信息无误? 确认后将无法修改！', '温馨提示', {
          ...
          type: 'warning'
            ...
            this.$http.post(api_prefix + "/Order/reviewVoucher", this.multipleSelection).then(
                response => {
                if (response.data.code == 0) {
                    setTimeout(() => {
                    this.closeAlert();
