    ...
    <div class="jdy-content-inner-trip">
      ...
      <div class="jdy-tab">
        <el-dropdown split-button type="defult" class="fr mr10 mt10">
          <a id="to-line-page" href="#/declareConfig/declareConfigLine">添加产品</a>
        ...
        <el-button type="default" class="fright mt10 mr10 btnInTab" @click="updatePage" v-loading.fullscreen.lock="fullscreenLoading">刷新</el-button>
        <el-button type="default" class="fright mt10 mr10 btnInTab" @click="alertJournal">日志</el-button>
      ...
      <div class="jdy-searchPart">
        ...
        <div class="inlineblock mr10">
          <el-input placeholder="产品名称、产品编号、拼音码" v-model="searchParams.pName" class="" style="width:400px">
      ...
      <div class="jdy-table p10">
        <el-table :data="tableData" border class="all" @row-click="rowclickHandler" highlight-current-row>
          ...
          <el-table-column label="产品经理" width="100">
            <template scope="scope" >
          ...
          <el-table-column label="状态" width="100">
            <template scope="scope">
              ...
              <span>{{ scope.row.pStatus|statuefilter }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" min-width="80">
            <template scope="scope">
              <el-select v-model="editType[scope.row.id]" @change="gotoEditPage" size="mini" class="goto-update" placeholder="编辑" value="">
                ...
                <el-option key="8" value="8" label="预算成本"></el-option>
                <el-option key="3" value="3" label="出厂价信息"></el-option>
              ...
              <el-button type="default" size="mini" v-show="scope.row.pStatus==4" @click="submit(scope.row.companyId,scope.row.id, scope.row.updateUser)">提交</el-button>
...
export default {
  ...
  methods: {
    ...
    gotoEditPage(val) {
      ...
      if (val == 1) { location.href = "/transfer.html#/declareConfig/declareConfigLine?id=" + this.pid+"&c_id="+this.c_id; return; }
      if (val == 2) { location.href = "/transfer.html#/declareConfig/declareConfigTrip?id=" + this.pid+"&c_id="+this.c_id; return; }
      if (val == 3) { location.href = "/transfer.html#/declareConfig/declareConfigTicketPrice?id=" + this.pid+"&c_id="+this.c_id; return; }
      if (val == 4) { location.href = "/transfer.html#/declareConfig/declareConfigSchedule?id=" + this.pid+"&c_id="+this.c_id; return; }
      if (val == 5) { if (this.tripDays < 1) { this.$alert("请先添加默认行程"); return } location.href = "/transfer.html#/list/supplement?id=" + this.pid + '&pDays=' + this.tripDays + '&isAdd=' + true+"&c_id="+this.c_id; return; }
      if (val == 6) { this.alertSetkeyFlag = true; }
      if (val == 8) { location.href = "/transfer.html#/declareConfig/declareConfigAddCostList?id=" + this.pid+"&c_id="+this.c_id; return; }
      if (val == 9){
        this.getContract()
      }
      if (!this.pid) {
      ...
      if (val == 7) { location.href = "#/declareConfig/declareConfigLine?copyId=" + this.pid+"&c_id="+this.c_id }
    },
    // 合同模板-查询
    getContract(){
   ...
   if( params.pType=='0'){
    ...
    submit(companyId,id,updateUser){
        this.$http.post(api_prefix + 'product/uneffect',
          {
            "companyId": companyId,
            "id": id,
