<template>
    <div class="jdy-content jdy-transfer fleft">
        <div class="jdy-content-inner">
            <el-form :model="tickForm" :rules="rules" ref="tickForm" class="demo-form-inline" label-width="110px">
                <div class="jdy-tab">
                    <a style="border:0;">{{productDetail.pName}}
                        <span></span>
                    </a>
                    <el-button class="fright mt10 mr10" @click="goback('ProductDeployTicketprice')">返回</el-button>
                    <el-button type="primary" @click="onSubmit()" class="fright mt10 mr10" :disabled="btnFlag">保存</el-button>
                </div>
                <div class="headerTip">请选择出厂价：</div>
                <el-table :data="tableData" class="all" @selection-change="handleSelectionChange" ref="multipleTable">
                    <el-table-column
                        type="selection"
                        width="55">
                    </el-table-column>
                    ...
                    <el-table-column label="出厂价类型" min-width="150" header-align="center" align="center">
                    <template scope="scope">
                        {{ scope.row.type == 0?"成人票":"儿童票" }}
                    </template>
                    </el-table-column>
                    <el-table-column label="出厂价名称" min-width="150" header-align="center" align="center">
                    <template scope="scope">
                        {{ scope.row.ticketName }}
                    </template>
                    </el-table-column>
                    <el-table-column label="预算成本" min-width="150" header-align="center" align="center">
                    <template scope="scope">
                        {{ scope.row.costPrice }}
                    </template>
                    </el-table-column>
                    <el-table-column label="出厂价" min-width="150" header-align="center" align="center">
                    <template scope="scope">
                        {{ scope.row.price }}
                    </template>
                    </el-table-column>
                    <el-table-column label="适用时间" min-width="150" header-align="center" align="center">
                    <template scope="scope">
                        {{ scope.row.suitableStartTime }} - {{ scope.row.suitableEndTime }}
                    </template>
                    </el-table-column>
                    <el-table-column label="始发站" min-width="350" header-align="center" align="center">
                    <template scope="scope">
                        {{ scope.row.departureNames | filterDeparture }}
                    </template>
                    </el-table-column>
                </el-table>
                <div class="tripbox clearfix border relative mt10 pt30">
                    <div class="clearfix p10">
                        ...
                        <el-row>
                            ...
                            <el-col :span="6">
                                <el-form-item label="预算成本价:" class="redstar">
                                    <el-input class="inputGeneral" v-model="costPrice" :disabled="true"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="6">
                                <el-form-item label="出厂价:" class="redstar">
                                    <el-input class="inputGeneral" v-model="tickForm.factoryPrice" :disabled="true"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="6">
                                ...
                                <el-form-item label="始发站:" class="redstar">
                                    <div class="tfcity relative">
                                        <div class="el-input" id="SFstation">
                                            <el-select v-model="selectStationArr" multiple filterable placeholder="请选择始发站" :disabled="true">
                        ...
                        <!-- <el-row>
                            <el-col :span="6" v-if="productDetail.ascription==1">
...
export default {
    ...
    data() {
        return {
            ...
            tickForm: {
                ...
                "tType": 0,
                "factoryTicketIds":[],//
                "suitableStartTime":'',//适用时间
                "suitableEndTime":'',//适用时间
                "factoryPrice":'',//出厂价
                "costPrice":'',//预算成本价
                "tName": '成人票',//票价名称
                "tLimitType": 0,//限制类型
    ...
    methods: {
        ...
        handleSelectionChange(val) {
            ...
            this.tickForm.factoryTicketIds = [];
            this.tickForm.factoryPrice = 0;
            this.selectStationArr = [];
            for(let i = 0;i<this.multipleSelection.length;i++){
                this.tickForm.factoryTicketIds.push(this.multipleSelection[i].id)
                this.tickForm.factoryPrice += Number(this.multipleSelection[i].price)
                this.multipleSelection[i].departureIds.forEach((data)=>{
                    this.selectStationArr.push(data);
                });
            }
            this.tickForm.factoryTicketIds = this.tickForm.factoryTicketIds.unique()
            this.selectStationArr = this.selectStationArr.unique()
        },
        ...
        getPrice(){
            ...
            this.$http.get(api_prefix + 'productCost/selectNewestCostUnitPriceByProductId/'+this.$route.query.id).then(response => {
                if(response.data.code == 0){
                    this.costPrice = response.data.body;
                }else{
            ...
            if(this.$route.query.type==1){
                this.$http.get(api_prefix + 'ticket/get/'+this.$route.query.priceId+"/0").then(response => {
                    if(response.data.code == 0){
                        ...
                        this.formatEdit();
                        let factoryTicketIds = this.tickForm.factoryTicketIds;
                        // let departureList = this.tickForm.departureList
                        for(let k = 0;k<factoryTicketIds.length;k++){
                            this.tableData.forEach((row, index) => {
                                if(factoryTicketIds[k] == row.id){
                        ...
                        this.multipleSelection.forEach((data)=>{
                            data.departureIds.forEach((dataTwo)=>{
                                this.selectStationArr.push(dataTwo);
                            });
                        });
                        console.log(this.selectStationArr,'this.selectStationArr')
                        this.usefulDate = [this.tickForm.suitableStartTime,this.tickForm.suitableEndTime]
                        console.log(this.usefulDate,'this.usefulDate')
        ...
        getName(value){
            ...
            console.log(this.tickForm.tAdultNum,'090909090')
            console.log(this.tickForm.tChildNum,'090909090')
        },
        // 星期全选相关
        ...
        onSubmit(){
            ...
            if(valid){
                ...
                if(data.code == "0"){
                    ...
                    this.$router.push({ name: 'ProductDeployTicketprice',query:{id: this.$route.query.id,c_id:this.$route.query.c_id}})
                }else{
                    this.btnFlag = false;
