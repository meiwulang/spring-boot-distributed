    ...
    <div class="jdy-content jdy-transfer fleft noborder">
        ...
        <el-form ref="ruleForm" label-width="20px" :inline="true" class="gz-itemMragin">
          <el-row>
            ...
            <el-form-item label="快速搜索：" prop="quickSearch" label-width="84px">
              <el-input v-model="searchVal" ref="serachIpt" placeholder="用户名、拼音码、真实名字、手机号" class="gz-largeWidth">
              </el-input>
            </el-form-item>
      ...
      <div class="jdy-table">
        <el-table :data="tableData" border highlight-current-row @row-click="rowclickHandler" style="width: 100%">
          ...
          <el-table-column prop="cName" label="单位名称" width="400">
          </el-table-column>
          <el-table-column label="操作">
            <template scope="scope">
              <el-button @click="editInfo(tableData,scope.row.userId)" type="default" size="mini">编辑</el-button>
...
export default {
  ...
  methods: {
    //初始化获取用户列表
    defaultUserList() {
      this.$http.post(api_prefix + '/user/list', this.userAlldata).then(response => {
    ...
    firstSearch() {
      ...
      this.userAlldata.uAccount = this.searchVal
      this.userAlldata.uRealName = this.searchVal
      this.userAlldata.uPym = this.searchVal
      this.userAlldata.uTel = this.searchVal
    ...
    //编辑部门信息
    editInfo(data, index) {
      this.$router.push({ path: '/userinfo', query: { formDataIndex: index, isAdd: false } })
    },
    //currentPage 改变时会触发
    ...
    //添加用户
    addInfoRouter() {
      this.$router.push({ path: '/userinfo', query: { isAdd: true } })
    },
