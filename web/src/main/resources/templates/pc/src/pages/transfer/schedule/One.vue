        ...
        <div class="tripbox clearfix  relative mt60">
          <div v-if="addType!=2" class="clearfix p10 border pb30">
            ...
            <el-row>
              ...
              <!--<el-col :span="8">-->
              <!--<el-form-item label="车辆数:" prop="sCarNum" >-->
              <!--<el-input-number v-model.number="banqiForm.sCarNum" placeholder="请输入车辆数" :min="0" :max="200" style="width: 100%;" :controls="false" @change="carNumChange"></el-input-number>-->
              <!--</el-form-item>-->
              <!--</el-col>-->
              <!--<el-col :span="8">-->
              <!--<el-form-item label="车座位数:" prop="sCarSeats" >-->
              <!--<el-input-number v-model.number="banqiForm.sCarSeats" placeholder="请输入车座位数" :min="0" :max="200" style="width: 100%;" :controls="false"></el-input-number>-->
              <!--</el-form-item>-->
              <!--</el-col>-->
              <el-col :span="8">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="对号入座:" prop="">
                  <el-select v-model.number="banqiForm.sSitType" placeholder="请选择" class="all">
          ...
          <div v-if="addType==2" class="clearfix p10 border pb30">
            ...
            <el-row>
              <el-col :span="18">
                ...
                <!--<el-col :span="12">-->
                <!--<el-form-item label="车辆数:" prop="sCarNum">-->
                <!--<el-input-number v-model.number="banqiForm.sCarNum" placeholder="请输入车辆数" :min="0" :max="200"-->
                <!--style="width: 100%;" :controls="false" @change="carNumChange"></el-input-number>-->
                <!--</el-form-item>-->
                <!--</el-col>-->
                <!--<el-col :span="12">-->
                <!--<el-form-item label="车座位数:" prop="sCarSeats">-->
                <!--<el-input-number v-model.number="banqiForm.sCarSeats" placeholder="请输入车座位数" :min="0" :max="200"-->
            ...
            <div class="p10 border pb30 schedule-one">
              ...
              <el-table ref="multipleTable" :data="ticketList" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="适用站点" width="">
                  <template scope="scope">{{ scope.row.departureList.length }}个站点</template>
                </el-table-column>
              ...
              <div class="clearfix">
                <el-pagination class="fright mt20" @size-change="pageSizeChange" @current-change="currentPageChange" :current-page.sync="ticketPage.currPage" :page-size="ticketPage.pageSize" layout="prev, pager, next, jumper" :total="ticketPage.total">
                </el-pagination>
              </div>
            </div>
          </div>
              ...
              <el-table :data="tableData2" border tooltip-effect="dark" style="width: 100%" class="myclass">
                <el-table-column label="调整车辆数" width="200" align="center">
                  <template scope="scope">
                    <el-row class="pb10">
                      ...
                      <el-col :span="4">
                        <el-button class=" ml10 carboxss-button mt10" @click="addCar">
                          <i class="el-icon-plus blue transx07"></i>新增
                        </el-button>
                        <el-button class="mt11 carboxss-button" @click="removeCar">
                    ...
                    <el-row class="pb10">
                      <el-col :span="18">
                        <el-form-item label="当前车号:" prop="">
                          <el-input :value="currentCar>-1?currentCar+1:''" :readonly="true" placeholder="请选择车辆"></el-input>
                        </el-form-item>
                        <el-form-item label="新座位数:" prop="">
                          <el-input-number :disabled="currentCar==-1" v-model.number="currentCar==-1?null:carList[currentCar].bSeatsNum" placeholder="请选择车辆" @change="seatChange" style="width: 100%;"></el-input-number>
                        </el-form-item>
                      </el-col>
                      <!--<el-col :span="6">-->
                    ...
                    <el-row class="pb10">
                      <el-col :span="18">
                        <el-form-item label="选择单位:">
                          <el-select v-model="currentCompany" class="all" clearable filterable remote placeholder="请选择单位" :remote-method="searchCompany" :loading="loading">
                            <el-option :key="item.id" v-for="item in companyList" :label="item.cName" :value="item.id">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="预留时效(h):" prop="">
                          <el-input v-model="temptoraryTime" type="number" placeholder="请输入预留时效"></el-input>
                        </el-form-item>
                      </el-col>
                      <el-col :span="6">
                        <el-button class="mt10 ml10 carboxss-button" @click="addTemporary">
                          <i class="icon3 inlineblock icon3-stay "></i>预留
                        </el-button>
                        <el-button class="mt11 carboxss-button" @click="releaseTemporary">
                ...
                <el-table-column label="其他管理" width="" align="center">
                  <template scope="scope">
                    ...
                    <el-col :span="9">
                      <div class="carboxss-b tc" @click="lockSeat">
                        <i class="icon3 inlineblock icon3-suo1 mt14 mb3"></i>
                        <p class="mt010">锁定车位</p>
                      </div>
                    </el-col>
                    <el-col :span="9">
                      <div class="carboxss-b tc ml10" @click="unlockSeat">
                  ...
                  <el-row style="height:500px">
                    <el-col :span="16">
                      <div class="carboxbb-table">
                        <el-table ref="singleTable" :data="carList" :show-header="false" highlight-current-row border height="250" @current-change="handleCurrentChange" style="width: 100%">
                          <el-table-column property="bus" label="bus" width="120">
                            <template scope="scope">BUS<input class="sortNum" hidden :value="scope.$index+1" />
                            </template>
                          </el-table-column>
                          <el-table-column property="info" label="信息">
                            <template scope="scope">
                              <div>
                                {{banqiForm.sScheduleNo}} {{scope.$index + 1}}号车[{{formatDate(calendars, "yyyy-MM-dd")}}]
                              </div>
                              <div>总数：{{carList[scope.$index].bSeatsNum}}
                                <span class="ml10">已售：{{ banqiForm.seatSold ? banqiForm.seatSold : 0}}</span>
                              </div>
                            </template>
                          </el-table-column>
                        </el-table>
                    ...
                    <el-col :span="8">
                      <div class="carboxbb-right" v-if="currentCar>-1">
                        <template v-for="n in carList[currentCar].bSeatsNum">
                          <el-tooltip :content="getTipContent(n)" placement="top" effect="light" :disabled="!(carList[currentCar] && carList[currentCar].holdList.indexOf(n.toString())!=-1)">
                            <div class="icon3 icon3-seat inlineblock" :class="{'icon3-seat-temporary':carList[currentCar] && carList[currentCar].holdList.indexOf(n.toString())!=-1,'icon3-seat-lock':carList[currentCar] && carList[currentCar].bSeatsLock.split(',').indexOf(n.toString())!=-1,'icon3-seat-success':selectedList.indexOf(n)!=-1}" @click="setSeat(n)">{{n}}
                            </div>
                          </el-tooltip>
                        </template>
...
//          this.carNumChange(this.banqiForm.sCarNum);
        ...
        if (valid) {
          ...
          this.$http.post(this.saveUrl, this.banqiForm).then(response => {
            if(response.data.code==0){
              ...
              setTimeout(() => {
                this.btnFlag = false;
                this.$router.push({ name: "Schedule", query: { id: this.$route.query.id } });
              }, 1000)
            }else{
              this.btnFlag = false;
              this.$alert(response.data.message);
    ...
    dealBanqiFormData() {   //将临时变量赋值为表单对象
      ...
      this.$refs.multipleTable.selection.forEach((value) => {
        ...
        if (!value.tPeerPrice || value.tPeerPrice <= 0) {
          this.$alert("同行价不能为空且非负", '温馨提示', { type: "warning" });
          errorFlag = true;
        }
      ...
      if (this.carList.length == 0) {
        this.$alert("请先添加车辆信息", '温馨提示', { type: "warning" });
        return false;
      }
      for (let i = 0; i < this.carList.length; i++) {
        if (this.carList[i].bSeatsNum <= 0) {
          this.$alert("请为" + (i + 1) + '号车添加座位', '温馨提示', { type: "warning" });
          return false;
        }
      }
