<template>
	<div class="jdy-content jdy-resources fleft">
		<div class="jdy-content-inner-trip">
			<el-row class="jdy-content-inner-trip pl10 pt10 pr10">
				<inner-header :activename="headerData.name" :savebtn="headerData.btnFlag" :showTripTab="headerData.showBtnInTabFlag" title="添加成本" @savedata="router"></inner-header> 
			</el-row>				
	...
	methods: { 		
        ...
        getDetail(id){
			if(this.locationFlag=="assemblylist"){
				...
				this.$router.push({name:'declareConfigAddCostPage',query:{id:id,type:1,pid:this.pid,c_id:this.c_id}})
			}			
		...
		router(){
			if(this.locationFlag=='assemblylist'){
				this.$router.push({name:'assemblyAddCostPage',query: {type: 0,pid:this.pid,isNull:this.isNull,c_id:this.c_id}})
			}else{
				this.$router.push({name:'declareConfigAddCostPage',query: {id:this.$route.query.id,type: 0,pid:this.pid,isNull:this.isNull,c_id:this.c_id}})
			}
		}
	},
	mounted(){
