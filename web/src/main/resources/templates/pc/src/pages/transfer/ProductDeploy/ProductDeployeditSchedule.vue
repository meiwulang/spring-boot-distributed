  ...
  <div class="jdy-content jdy-transfer fleft">
    <div class="jdy-content-inner">
      <el-form :model="banqiForm" :rules="banqiRule" ref="banqiForm" class="demo-form-inline" label-width="100px">
        <div class="jdy-tab border">
          <el-button @click="goback('ProductDeploySchedule')" class="fright mt10 mr10">返回</el-button>
          <el-button type="primary" @click="onSubmit('banqiForm')" class="fright mt10 mr10" :disabled="btnFlag">保存</el-button>
        </div>
        <div class="tripbox clearfix relative mt30">
                ...
                <el-form-item label="出发时分:" prop="">
                  ...
                  <template>
                    <el-time-picker v-model="sLeaveTime" :picker-options="{
                            format: 'HH:mm'
                          }" format="HH:mm" placeholder="出发时分" clearable :editable="false" style="width: 100%;" :disabled='true'>
                    </el-time-picker>
                  </template>
                </el-form-item>
              </el-col>
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="成本预算:">
                  <el-input-number v-model="banqiForm.priceNum" style="width: 100%;" :controls="false" :disabled='true'></el-input-number>
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <div class="clarfix alertAddProduct2-table pb30 mt30">
              ...
              <el-table ref="multipleTable" :data="ticketList" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="" label="门市价" width="">
                  <template scope="scope">
                    <el-input-number v-model="scope.row.tMarketPrice" :controls="false" :min="0" :max="1000000"></el-input-number>
                  </template>
                </el-table-column>
                <el-table-column prop="" label="出厂价" width="">
                  <template scope="scope">
                    <el-input-number v-model="scope.row.factoryPrice" :controls="false" :min="0" :max="1000000" :disabled="true"></el-input-number>
                  </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    getPrice(val){
        // 新增票价需要调查询成本单价接口
        this.$http.post(api_prefix + '/productCosting/selectNewestCostUnitPriceByProductIdAndDate',{
          date: val,
          productId: this.$route.query.id
        }).then(response => {
            if(response.data.code == 0){
                this.banqiForm.priceNum = response.data.body;
            }else{
        ...
        if (valid) {
          ...
          this.$http.post(this.saveUrl, this.banqiForm).then(response => {
            if(response.data.code==0){
              ...
              setTimeout(() => {
                this.btnFlag = false;
                this.$router.push({ name: "ProductDeploySchedule", query: { id: this.$route.query.id,c_id: this.$route.query.c_id} });
              }, 1000)
            }else{
              this.btnFlag = false;
    ...
    defaultBanqiForm() {
      this.dealHttpReseponse(this.getScheduleUrl, null, (response) => {
        ...
        this.getPrice(banqiForm.sCalendar);
