        ...
        this.$http.post(api_prefix + "trip/update", this.ruleForm).then(
          response => {
            if (response.data.code == 0) {
              ...
              this.getTripList();
              this.$router.push({ name: "declareConfigAddCostList",query:{id: this.$route.query.id,c_id:this.$route.query.c_id,tripFlag:this.$route.query.tripFlag}})
            } else {
              this.btnFlag = false;
              this.$alert(response.data.message, "温馨提示", {
                confirmButtonText: "确定",
