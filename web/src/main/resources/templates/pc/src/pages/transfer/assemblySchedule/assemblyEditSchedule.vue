  ...
  <div class="jdy-content jdy-transfer fleft">
    <div class="jdy-content-inner">
      <el-form :model="banqiForm" :rules="banqiRule" ref="banqiForm" class="demo-form-inline" label-width="100px">
        <div class="jdy-tab">
          <el-button @click="goback('assemblySchedule')" class="fright mt10 mr10">返回</el-button>
        ...
        <div class="tripbox clearfix  relative mt60">
          <div v-if="addType!=2" class="clearfix p10 border pb30">
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="班期名称:">
                  <el-input v-model="banqiForm.sScheduleName" placeholder="请输入班期名称" :disabled='true'>
                  </el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="出发时分:" prop="">
                  ...
                  <template>
                    <el-time-picker v-model="sLeaveTime" :picker-options="{
                            format: 'HH:mm'
                          }" format="HH:mm" placeholder="出发时分" clearable :disabled='true' :editable="false" style="width: 100%;">
                    </el-time-picker>
                  </template>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="停售时间:" prop="sStopSale">
                  <el-input placeholder="请输入内容" v-model.number="banqiForm.sStopSale" type="number" :disabled='true'>
                    <el-select v-model="banqiForm.sStopType" slot="prepend" placeholder="请选择" style="width: 100px;" :disabled='true'>
                      <el-option label="分钟" :value='0'></el-option>
                      <el-option label="小时" :value='1'></el-option>
                      <el-option label="天" :value='2'></el-option>
                    </el-select>
                  </el-input>
                  <!--<el-select v-model="banqiForm.endTime" placeholder="请选择星期几" class="all">-->
                  <!--<el-option :key="0" label="提前30分钟" value="0">-->
                  <!--</el-option>-->
                  <!--<el-option :key="1" label="提前15分钟" value="1">-->
                  <!--</el-option>-->
                  <!--</el-select>-->
                </el-form-item>
              </el-col>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="虚拟已售数:">
                  <el-input-number v-model.number="banqiForm.sShamNum" placeholder="虚拟已售数" :min="0" :max="100" style="width: 100%;" :controls="false" :disabled='true'></el-input-number>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="对号入座:" prop="">
                  <el-select v-model="banqiForm.sSitType" placeholder="请选择" class="all"  @change='changeType' :disabled='true'>
              ...
              <!--<el-select v-model="banqiForm.sPrint" style="width: 100%;">-->
              <!--<el-option :key="key" label="value" :value="key" v-for="(value,key) in setType">-->
              <!--</el-option>-->
              <!--</el-select>-->
              <!--</el-form-item>-->
              <!--</el-col>-->
              <el-col :span="8">
                <el-form-item label="成团人数:">
                  <el-input-number v-model="banqiForm.sGroupNum" :min="1" :max="100" style="width: 100%;" :controls="false" :disabled='true'></el-input-number>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item label="出发地:">
                  <el-input v-model="banqiForm.chufa" style="width: 100%;" :disabled='true'></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="始发站:">
                  <el-input v-model="banqiForm.shifa" style="width: 100%;" :disabled='true'></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </div>
          ...
          <div class="clarfix alertAddProduct2-table pb30 mt60">
            <div class="p10 border pb30 schedule-one">
              ...
              <el-table ref="multipleTable" :data="ticketList" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
                <el-table-column type="selection" width="55">
                </el-table-column>
                <el-table-column label="适用站点" width="">
                  <template scope="scope">{{ scope.row.departureNum}}个站点</template>
                </el-table-column>
                ...
                <el-table-column prop="" label="门市价" width="">
                  <template scope="scope">
                    <el-input-number v-model="scope.row.tMarketPrice" :controls="false" :min="0" :max="1000000" :disabled="true"></el-input-number>
                  </template>
                </el-table-column>
                <!-- <el-table-column prop="" label="同行价" width="">
                  <template scope="scope">
                    <el-input-number v-model="scope.row.tPeerPrice" :controls="false" :min="0" :max="1000000"></el-input-number>
                  </template>
                </el-table-column> -->
                <!-- <el-table-column prop="" label="周几有效" width="">
                  <template scope="scope">{{ getEffctiveDay(scope.row.tEffectWeek) || "无" }}</template>
                </el-table-column> -->
                <el-table-column prop="" label="库存量（-1代表无限制）" width="">
                  <template scope="scope">
                    <el-input-number v-model="scope.row.tStock" :controls="false" :min="-1" :max="1000000" :disabled="true"></el-input-number>
                  </template>
                </el-table-column>
                <!-- <el-table-column prop="" label="预置类型" width="">
                  <template scope="scope">{{ scope.row.tTicketType == 1 ? "套票":"单票" }}</template>
                </el-table-column>
                <el-table-column prop="" label="限制类型" width="">
                  <template scope="scope">{{ getLimitType(scope.row.tLimitType) }}</template>
                </el-table-column>
                <el-table-column prop="" label="限制条件" width="">
                  <template scope="scope">{{ scope.row.tLimitCondition || "--" }}</template>
                </el-table-column> -->
              </el-table>
              <!--<div class="clearfix">-->
              <!--<el-pagination class="fright mt20" @size-change="pageSizeChange" @current-change="currentPageChange"-->
              <!--:current-page.sync="ticketPage.currPage" :page-size="ticketPage.pageSize"-->
              <!--layout="prev, pager, next, jumper" :total="ticketPage.total">-->
              <!--</el-pagination>-->
              <!--</div>-->
            </div>
          </div>
          <div class="clarfix alertAddProduct2-table pb30 mt30 carboxss">
            <div class="p10 border pb30 schedule-one">
              ...
              <el-table :data="tableData2" border tooltip-effect="dark" style="width: 100%" class="myclass">
                <el-table-column label="调整车辆数" width="200" align="center">
                  <template scope="scope">
                    <el-row class="pb10">
                      <el-col :span="5">
                        &nbsp;
                      ...
                      <el-col :span="4">
                        <el-button class=" ml10 carboxss-button mt10" @click="addCar" :disabled="true">
                          <i class="el-icon-plus blue transx07"></i>新增
                        </el-button>
                        <el-button class="mt11 carboxss-button" @click="removeCar" :disabled="true">
                          <i class="el-icon-close red  transx06"></i>删除
                        </el-button>
                      </el-col>
                    </el-row>
                  </template>
                ...
                <el-table-column label="调整座位数" width="" align="center">

                  <template scope="scope">
                    <el-row class="pb10">
                      <el-col :span="18">
                        <el-form-item label="当前车号:" prop="">
                          <el-input :value="currentCar>-1?currentCar+1:''" :readonly="true" placeholder="请选择车辆" :disabled='true'></el-input>
                        </el-form-item>
                        <el-form-item label="新座位数:" prop="">
                          <el-input-number :disabled="true" v-model.number="currentCar==-1?null:currentCarSeatNum" placeholder="请选择车辆" @change="seatChange" style="width: 100%;" ></el-input-number>
                        </el-form-item>
                      </el-col>
                      <el-col :span="6">
                        <div class="border caredit1 tc" style="background-color:#eef1f6">
                ...
                <el-table-column label="预留座位数" width="" align="center">
                  <template scope="scope">
                    <el-row class="pb10">
                      <el-col :span="18">
                        <el-form-item label="选择单位:">
                          <el-select v-model="currentCompany" class="all" clearable filterable remote placeholder="请选择单位" :remote-method="searchCompany" :loading="loading" :disabled='true'>
                            <el-option :key="item.id" v-for="item in companyList" :label="item.cName" :value="item.id">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="预留时效(h):" prop="">
                          <el-input v-model="temptoraryTime" type="number" placeholder="请输入预留时效" :disabled='true'></el-input>
                        </el-form-item>
                      </el-col>
                      <el-col :span="6">
                        <el-button class="mt10 ml10 carboxss-button" @click="addTemporary" :disabled='true'>
                          <i class="icon3 inlineblock icon3-stay "></i>预留
                        </el-button>
                        <el-button class="mt11 carboxss-button" @click="releaseTemporary" :disabled='true'>
                          <i class="icon3 inlineblock icon3-discharge"></i>释放
                        </el-button>
                      </el-col>
                    </el-row>
                  </template>
                </el-table-column>
                <el-table-column label="其他管理" width="" align="center">
                  <template scope="scope">
                    <el-col :span="3">&nbsp;</el-col>
                    <el-col :span="9">
                      <div class="carboxss-b tc" style="background-color:#eef1f6">
                        <i class="icon3 inlineblock icon3-suo1 mt14 mb3"></i>
                        <p class="mt010">锁定车位</p>
                      </div>
                    </el-col>
                    <el-col :span="9">
                      <div class="carboxss-b tc ml10" style="background-color:#eef1f6">
                        <i class="icon3 inlineblock icon3-suo2 mt14 mb3"></i>
                        <p class="mt010">解锁车位</p>
                      </div>
                    </el-col>
                    <!--<el-col :span="6">-->
                    <!--<el-button class=" ml10 carboxss-button" @click="addCar" :disabled="canNotModifyCarNum">-->
                    <!--<i class="el-icon-plus blue transx07" ></i>新增-->
                    <!--</el-button>-->
                    <!--<el-button class="mt11 carboxss-button"@click="removeCar" :disabled="canNotModifyCarNum">-->
                    <!--<i class="el-icon-close red  transx06"></i>删除-->
                    <!--</el-button>-->
                    <!--</el-col>-->
                  </template>
                  ...
                  <el-row style="height:500px">
                    <el-col :span="16">
                      <div class="carboxbb-table">
                        <el-table ref="singleTable" :data="carList" :show-header="false" highlight-current-row border height="500" @current-change="handleCurrentChange" style="width: 100%">
                          <el-table-column property="bus" label="bus" width="120">
                            <template scope="scope">BUS<input class="sortNum" hidden :value="scope.$index+1" /></template>
                          </el-table-column>
                          <el-table-column property="info" label="信息">
                            <template scope="scope">
                              <div>{{banqiForm.sScheduleNo}} {{scope.$index+1}}号车[{{formatDate(calendars,"yyyy-MM-dd")}}]</div>
                              <div>总数：{{carList[scope.$index].bSeatsNum}}
                                <span class="ml10">已售：{{carList[scope.$index].seatsSoldNum ? carList[scope.$index].seatsSoldNum:0}}</span>
                              </div>
                            </template>
                          </el-table-column>
                        </el-table>
                    ...
                    <el-col :span="8">
                      <div class="carboxbb-right" v-if="currentCar>-1">
                        <template v-for="n in carList[currentCar].bSeatsNum">
                          <el-tooltip :content="getTipContent(n)" placement="top" effect="light" :disabled="true">
                            <div class="icon3 icon3-seat inlineblock" al :class="{'icon3-seat-temporary':carList[currentCar] && carList[currentCar].holdList.indexOf(n.toString())!=-1,'icon3-seat-lock':carList[currentCar] && carList[currentCar].bSeatsLock.split(',').indexOf(n.toString())!=-1,'icon3-seat-success':selectedList.indexOf(n)!=-1}" @click="setSeat(n)">{{n}}
                            </div>
                          </el-tooltip>
                        </template>
                        <div style="margin-left: 100px;color: red" v-if="carList[currentCar].bSeatsNum<=0">请设置座位数</div>
                      </div>
                    </el-col>
                  </el-row>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      getScheduleUrl: api_prefix + "/Schedule/scheduleWithTickets/" + this.$route.query.banqi + "/" + 1,
        ...
        if (valid) {
          ...
          this.$http.post(this.saveUrl, this.banqiForm).then(response => {
            if(response.data.code==0){
              ...
              setTimeout(() => {
                this.btnFlag = false;
                this.$router.push({ name: "assemblySchedule", query: { id: this.$route.query.id } });
              }, 1000)
            }else{
              this.btnFlag = false;
    ...
    dealBanqiFormData() {   //将临时变量赋值为表单对象    1，开始时间和出发时间  2，判断票是否改变，3如果2成立计算新票，
      ...
      * */
      ...
      this.$refs.multipleTable.selection.forEach((value) => {
        if (!value.tMarketPrice || value.tMarketPrice <= 0) {
          this.$alert("门市价不能为空且非负", '温馨提示', { type: "warning" });
          errorFlag = true;
        }
        value.tPeerPrice = value.tMarketPrice;
        value.chosen = 1;
        selectedTicketArray.push({
          id: value.id,// 主键id ,
          tMarketPrice: value.tMarketPrice,//市场价 ,
          tPeerPrice: value.tPeerPrice,// 同行价 ,
          tStock: value.tStock,// 库存,
          choose: 1
        });
      });
      if (errorFlag) {
