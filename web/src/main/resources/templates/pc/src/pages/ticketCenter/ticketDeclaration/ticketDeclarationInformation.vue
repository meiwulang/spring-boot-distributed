<template>
  <div class="jdy-content jdy-transfer fleft">
    ...
    <div class="jdy-content-inner">
      <inner-header :activename="headerData.name" :savebtn="headerData.btnFlag" :showBtnInTab="headerData.showBtnInTabFlag" @savedata="onSubmit('lineForm')" :title="headerData.title"></inner-header>
      ...
      <el-form class="" :model="lineForm" :rules="rules" ref="lineForm" label-width="100px">
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="门票编码：" prop="ticketProductCode">
                  <el-input v-model="lineForm.ticketProductCode" placeholder="请填写门票编码"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="门票名称：" prop="ticketProductName">
                  <el-input v-model="lineForm.ticketProductName" placeholder="门票名称"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
              <el-form-item label="门票类型：" prop="ticketProductType" placeholder="请选择门票类型">
                <el-select v-model="lineForm.ticketProductType" clearable placeholder="请选择门票类型" style="width: 100%;">
              ...
              <el-col :span="12">
                <el-form-item label="产品经理：" prop="productManager">
                  <el-select v-model.number="lineForm.productManager" class="all" :filterable="true" :remote-method="searchContacts" clearable remote>
        ...
        <el-row :gutter="20">
          <el-form-item label="入园须知：">
            <jdy-editor :content="lineForm.admissions" attr="admissions" @input="editorUpdate"></jdy-editor>
            <!-- <el-input v-model="lineForm.characteristic" type="textarea"></el-input> -->
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="费用包含：">
            <jdy-editor :content="lineForm.costInclude" attr="costInclude" @input="editorUpdate"></jdy-editor>
            <!-- <el-input v-model="lineForm.cost" type="textarea"></el-input> -->
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="预定规则：">
            <jdy-editor :content="lineForm.reservationRule" attr="reservationRule" @input="editorUpdate"></jdy-editor>
            <!--<el-input v-model="lineForm.nocost" type="textarea"></el-input> -->
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="退票规则：">
            <jdy-editor :content="lineForm.refundRules" attr="refundRules" @input="editorUpdate"></jdy-editor>
            <!-- <el-input v-model="lineForm.notice" type="textarea"></el-input> -->
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="补充说明：">
            <jdy-editor :content="lineForm.explanation" attr="explanation" @input="editorUpdate"></jdy-editor>
            <!-- <el-input v-model="lineForm.notice" type="textarea"></el-input> -->
          </el-form-item>
        </el-row>
        <el-row :gutter="20">
          <el-form-item label="景点介绍：">
            <jdy-editor :content="lineForm.attractions" attr="attractions" @input="editorUpdate"></jdy-editor>
            <!-- <el-input v-model="lineForm.notice" type="textarea"></el-input> -->
          </el-form-item>
        </el-row>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        productManager: [
          { required: true, type:'number', message: '请输入产品经理', trigger: 'blur' },
        ],
        ticketProductType: [
  ...
  methods: {
    ...
    queryUserList(companyId) {
      this.$http.post(api_prefix + '/user/list', { "uCompanyId": companyId, "uRealName": this.lineForm.productManager,"uStype":0 }).then(rsp => {
        if (rsp.body.code == 0) {
    ...
    getBaseList(){
      this.$http.get(api_prefix + '/admission/base/getAdmissionBase/' + this.$route.query.id).then(response => {
        if (response.data.code == 0) {
          ...
          this.lineForm = data;
          console.log(this.headerData.showBtnInTabFlag,'this.showBtnInTabFlag')
        } else {
        ...
        if (valid) {
          ...
          this.$http.post(api_prefix + '/admission/base/saveAdmissionBase', this.lineForm).then(response => {
            if (response.body.code == 0) {
              ...
              this.$route.query.id = response.body.id[0];
