      ...
      <div class="ticketprice clearfix border relative">
        <div class="jdy-search jdy-search-bg2 clearfix">
          ...
          <div class="inlineblock mr10">
            <el-date-picker v-model="datePicker" type="daterange" placeholder="请选择查询日期范围" @change="dateRangeChange">
            </el-date-picker>
            <el-select v-model="schedulesStatus" placeholder="请选择班期状态" @change="changeSelectStatus" style="vertical-align: top;" class="ml10">
        ...
        <div class="jdy-table p10">
          <el-table :data="tableData" border class=" all" highlight-current-row @row-click="rowclickHandler">
            ...
            <el-table-column label="班期编号" min-width="150">
              <template scope="scope">
                {{ scope.row.sScheduleNo }}
              </template>
            </el-table-column>
            <el-table-column label="团号" min-width="150">
              <template scope="scope">
                {{ scope.row.sGroupOrderNo }}
              </template>
            </el-table-column>
            <el-table-column label="班期名称" min-width="150">
              <template scope="scope">
                {{ scope.row.sScheduleName }}
              </template>
            </el-table-column>
            <el-table-column label="出发日期" min-width="150">
              <template scope="scope">
                {{formatDate(scope.row.sCalendar)}}
              </template>
            </el-table-column>
            <el-table-column label="停售时间" min-width="150">
              <template scope="scope">
                {{getStopSaleDate(scope.row.sCalendar,scope.row.sLeaveTime, scope.row.sStopType, scope.row.sStopSale)}}
              </template>
            </el-table-column>
            <el-table-column label="成团人数" min-width="150">
              <template scope="scope">
                {{ scope.row.sGroupNum }}
              </template>
            </el-table-column>
            <el-table-column label="已售" min-width="150">
              <template scope="scope">
...
export default {
  ...
  methods: {
    ...
    editBanqi(id) {
      ...
      this.$router.push({ name: 'ProductDeployEditInventory', query: { id: this.search.sProductId, banqi: id,c_id:this.$route.query.c_id} })
    },
    editSeat(id) {   //清除自定义票价
