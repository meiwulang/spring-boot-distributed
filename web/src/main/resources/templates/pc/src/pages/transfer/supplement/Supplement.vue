    ...
    <div class="jdy-content-inner">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="">
        <div class="jdy-tab">
          ...
          <el-button type="primary" @click="onSubmit('ruleForm')" class="fright mt10 mr10">保存</el-button>
        </div>
        <!--jdy-content-trip end-->

        <div class="tripbox clearfix border relative mt30">
          <div class="tripbox-tab">
            <a title="" class="relative inlineblock fleft" href="javascript:;" @click="toLineInfo">线路信息
              <span class="absolute inlineblock"></span>
            </a>
            <a title="" class="relative inlineblock active fleft">行程信息
              <span class="absolute inlineblock"></span>
            </a>
          </div>
          <div class="tripbox-left tripbox-left-nobg absolute">
          ...
          <div style="padding: 20px 20px 0 0">
            <el-row>
              <el-form-item label="行程标题：" prop="title">
                <el-input v-model="ruleForm.tTitle" placeholder="请输入行程标题"></el-input>
              </el-form-item>
            </el-row>
            <el-row>
              <el-form-item label="时间选择：">
                <el-date-picker v-model="dateRange" :picker-options="pickerOptions" type="daterange" @change="datetimerange" placeholder="选择时间范围" style="width:100%;">
                </el-date-picker>
              </el-form-item>
            </el-row>
          </div>
          <template v-for="(tripsItem,index) in ruleForm.trips">
            <div class="tripbox-right mt30" @click="getIndex(index)">
              ...
              <el-row>
                <el-col :span="18">
                  <el-form-item label="出发地/目的地：" prop="address">
                    <input type="text" v-model="tripsItem.tFromTo" placeholder="出发地/目的地" :id="'focusInput'+index" class="el-input__inner">
                  </el-form-item>
                </el-col>
              ...
              <el-row>
                ...
                <el-col :span="16">
                  <el-input v-model="tripsItem.tMealsRemark" placeholder="备注" @change="getEatRemark" :maxlength="30"></el-input>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="16">
                  <el-form-item label="住宿情况：" prop="accommodation">
                    <div>
                      <el-select v-model="tripsItem.phHotelIds" multiple filterable no-data-text="请输入进行搜索" placeholder="请选择" :filter-method="getChangeLive" @change="pushHotelIds" style="width: 100%">
                ...
                <el-col :span="6">
                  <el-input v-model="tripsItem.tHotelRemark" placeholder="备注" @change="getRoomRemark" style="margin-left:10px;" :maxlength="30"></el-input>
                </el-col>
                <el-col :span="2">
                  <el-button class="btnbg fr" @click="UniteBtn(tripsItem.phHotelIds,tripsItem.hotel,tripsItem.tHotelRemark)">同步住宿信息</el-button>
                </el-col>
              </el-row>
              <el-row>
                <el-form-item label="游玩景点:">
                  <el-select v-model="tripsItem.phScenicIds" multiple filterable no-data-text="请输入进行搜索" placeholder="请输入景点名称，拼音码" :filter-method="getChangeView" style="width: 100%" @change="pushScenicIds">
              ...
              <el-row>
                <el-form-item label="简易行程:">
                  <el-input v-model="tripsItem.tSimpleTrip" type="textarea" placeholder="简易行程" @change="getEasyTrip"></el-input>
                </el-form-item>
              </el-row>
              <el-row>
              ...
              <el-row>
                <el-form-item label=" ">
                  <a href="javascript:;">
                    <p class="absolute bendip blue jl-picBox" @click.stop="alertTag(index)">图片库</p>
                  </a>
                  <div class="jl-localPic" v-show="tripsItem.attachs.length < 4">
                    <label :for="'fileUpload'+index">
                      <div class="el-upload el-upload--picture-card" style="color:#20a0ff">
                        <input type="file" name="file" :id="'fileUpload'+index" class="el-upload__input" v-on:change="changecover($event)">
                        本地上传
                      </div>
                    </label>
                  </div>
      ...
      <!--改变标签样式 begin-->
      <jdy-alert title="添加/编辑标签" @closeAlert="alertTag" v-if="alertTagFlag" class="alertTag" style="width:850px">
          ...
          <el-row class="alertTagTop">
            ...
            <el-col :span="8">
              <el-form-item label="名称：">
                <el-select v-model="addTagStyle.pAlbumId" filterable placeholder="请选择相册名称" :filter-method="getChangeaName" @change="getPicList">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        // address: [
        //   { required: true, message: '请输入出发地/目的地', trigger: 'blur' }
        // ]
      },
      accommodationOption: [],
      dialogImageUrl: '',
  ...
  mounted() {
    console.log(this.$route.query.tcStartDay,'this.$route.query.tcStartDay')
    console.log(this.$route.query.tcEndDay,'this.$route.query.tcEndDay')
    this.setDays();
    this.$nextTick(() => {
      this.ruleForm.pDays = parseInt(this.$route.query.pDays)
      ...
      if (this.$route.query.isAdd == false) {
        this.$http.post(api_prefix + '/trip/list', {
          tProductId: this.$route.query.id,
          tType: 1,
          tcEndDay: this.$route.query.tcEndDay.replace(/-/g, ""),
          tcStartDay: this.$route.query.tcStartDay.replace(/-/g, "")
        }).then(response => {
          if (response.data.code == 0) {
            this.ruleForm = response.data.body
            this.dateRange = [new Date(this.$route.query.tcStartDay), new Date(this.$route.query.tcEndDay)]
            this.ruleForm.pDays = this.$route.query.pDays;
            this.ruleForm.tType = 1;
            this.ruleForm.tProductId = this.$route.query.id;
            this.ruleForm.puserId = 1;
            console.log(this.dateRange, 'pppppp')
  ...
  methods: {
    toLineInfo() {
      if (this.dateRange.length < 2 || this.isAdd) {
        this.$message.error("请先完善行程信息~");
        return;
      }
      let query = Object.assign(this.$route.query, { tcStartDay: window.FormatDate(this.dateRange[0], 'yyyy-MM-dd'), tcEndDay: window.FormatDate(this.dateRange[1], 'yyyy-MM-dd') }, { isAdd: this.isAdd });
      this.$router.push({ name: 'SupplementLine', query: query })
    },
    datetimerange(value) {
      console.log(value, '时间范围')
      var trips = value.replace(/-/g, "").replace(/\s+/g, "");
      this.ruleForm.tcStartDay = trips.substring(0, 8)
      this.ruleForm.tcEndDay = trips.substring(8)
      console.log(this.ruleForm.tcStartDay, 'this.ruleForm.tcStartDay')
      console.log(this.ruleForm.tcEndDay, 'this.ruleForm.tcEndDay')
    },
    getIndex(index) {
    ...
    rebuildMeals(item) {
      ...
      if (tMeals == '111') {
        item.haveMeals = ["早餐", "午餐", "晚餐"]; return;
      }
      if (tMeals == '110') {
        item.haveMeals = ["早餐", "午餐"]; return;
      }
      if (tMeals == '011') {
        item.haveMeals = ["晚餐", "午餐"]; return;
      }
      if (tMeals == '101') {
        item.haveMeals = ["早餐", "晚餐"]; return;
      }

      if (tMeals == '001') {
        item.haveMeals = ["晚餐"]; return;
      }
      if (tMeals == '100') {
        item.haveMeals = ["早餐"]; return;
      }
      if (tMeals == '010') {
        item.haveMeals = ["午餐"]; return;
      }
    ...
    queryHotelList(companyId) {
      this.$http.post(api_prefix + 'hotel/list', {
        "companyId": 123,
        "currPage": 1,
        "hName": null,
        "pageSize": 15,
    ...
    getEatRemark(value) {//获取用餐情况备注
      this.eatRemark = value;
      console.log(this.eatRemark, 'this.eatRemark')
    },
    pushHotelIds(value) {//获取pushHotelIds
      console.log(value)
    },
    getRoomRemark(value) {//获取住宿情况备注
      this.roomRemark = value;
      console.log(this.roomRemark, 'this.roomRemark')
    },
    getEasyTrip(value) {//获取简易行程
      this.easyTripValue = value;
      console.log(this.easyTripValue, 'this.easyTripValue')
    },
    getChangeAddress(value) {//获取出发地/目的地
      this.addressValue = value;
      console.log(this.addressValue, 'this.addressValue')
    },
    pushScenicIds(value) {//获取pushScenicIds
      console.log(value)
    },
    getChangeView(val) {//获取景点
      this.$http.post(api_prefix + '/scenic_spot/list', {
        sName: val,
        sPym: val
      }).then(response => {
        if (response.data.code == 0) {
    ...
    getChangeLive(val) {//获取住宿情况
      ...
      this.$http.post(api_prefix + '/hotel/list', {
        ...
        if (response.data.code == 0) {
          ...
          console.log(this.ruleForm.trips[this.alertIndex].hotel, '2123421')
        } else {
          this.$alert(response.data.message, '温馨提示', {
            confirmButtonText: '确定',
            callback: action => { }
    ...
    getChangeaName(val) {//搜索相册名称获取相应相册名称
      console.log(val)
      this.$http.post(api_prefix + '/album/list', {
        aType: this.pType,
        aName: val
    ...
    getPicList(value) {//选中相册名称进行数据请求获取相册列表内容
      console.log(value)
    ...
    getPicName(value) {//获取图片名称
      console.log(value)
      this.picName = value;
    },
    getSearchPicList() {//点击查询
      this.isSearch = true;
      this.$http.post(api_prefix + '/attach/list', {
    ...
    saveToTrip() {
      ...
      console.log(this.ruleForm.trips[this.alertIndex].attachs.length, 'attachs.length')
      console.log(this.ruleForm.trips[this.alertIndex].attachs.length + this.picNameArry.length, 'yiqi.length')
      if (this.picNameArry.length + this.ruleForm.trips[this.alertIndex].attachs.length > 4) {
        this.$message.error('最多只能上传四张图片!');
      }
      if (this.picNameArry.length + this.ruleForm.trips[this.alertIndex].attachs.length == 4) {
        // this.showPlus[this.alertIndex] = false;
        Array.prototype.push.apply(this.ruleForm.trips[this.alertIndex].attachs, this.picNameArry);
    ...
    setDays() {
      ...
      for (let i = 0; i < pDays - 1; i++) {
        this.ruleForm.trips.push(JSON.parse(str))
      }
      console.log(this.ruleForm.trips, 'jjjjj')
      console.log(pDays, 'kkkklk')
    ...
    onSubmit() {
      console.log(this.ruleForm, 'popopop')
      if (this.isAdd == false) {
        console.log("更新")
        this.$http.post(api_prefix + 'trip/update', this.ruleForm).then(response => {
          if (response.data.code == 0) {
            this.$message({
              type: 'success',
              message: '保存成功!'
        ...
        console.log("保存")
        this.$http.post(api_prefix + 'trip/save', this.ruleForm).then(response => {
          if (response.data.code == 0) {
            this.$message({
              type: 'success',
              message: '保存成功!'
    ...
    changecover($event) {
      ...
      if ((files.length + this.ruleForm.trips[this.alertIndex].attachs.length) > 4) {
        console.log(files.length + this.ruleForm.trips[this.alertIndex].attachs.length, 'files.length + this.picNameArry.length')
        return;
      } else if ((files.length + this.ruleForm.trips[this.alertIndex].attachs.length) == 4) {
        console.log(files.length + this.ruleForm.trips[this.alertIndex].attachs.length, 'files.length + this.picNameArry.length')
      } else {
      }
    ...
    createImage(file) {
      ...
      this.$http.post(api_prefix + "/common/file/upload", formData, {
        ...
        console.log(this.ruleForm.trips[this.alertIndex].attachs, '图库2')
