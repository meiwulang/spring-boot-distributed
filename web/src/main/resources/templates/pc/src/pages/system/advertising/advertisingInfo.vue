  ...
  <div class="jl-information gz-paddingBottom">
    <div class="jdy-content jdy-transfer fleft noborder">
      <div class="winCenter">
        <el-form :model="saveForm" :rules="saveFormRules" ref="saveForm">
          ...
          <el-form-item label="投放范围:" label-width="93px">
      ...
      <el-button type="primary" class="adverBtn" @click="saveNewAD">保存</el-button>
    </div>
    </div>
    <!--投放城市-->
    <jdy-alert title="投放城市选择" @closeAlert="closecfCity" v-if="showcfCityFlag" class="alertJournal showtfcity">
      ...
      <div class="alertfoot1 clearfix">
        ...
        <el-button type="primary" class="fright mt10 mr10" @click="saveCC" :disabled="btnFlag">保存</el-button>
      </div>
    </jdy-alert>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      saveFormRules: {
        aPlace: [
          {
            required: true, type: 'number', message: '请选择广告位置', trigger: 'change'
          }
        ],
        aStartTime: [
          { required: true, type: 'date', message: '请选择有效时间', trigger: 'change' }
        ],
        aEndTime: [
          { required: true, type: 'date', message: '请选择有效时间', trigger: 'change' }
        ],
        aOpenType: [
          { required: true, type: 'number', message: '请选择打开方式', trigger: 'change' }
        ],
        aShow: [
          { required: true, type: 'number', message: '请选择是否显示', trigger: 'change' }
        ],
        attachUrl: [
          { required: true, type: 'string', message: '请添加图片', trigger: 'change,blur,clearFiles,abort' }
        ],
        areaList: [
          { required: true, type: 'object', message: '请选择投放市场', trigger: 'change,blur' }
        ]
      },
      saveForm: {
        "attachUrl": null,
  ...
  mounted() {
    ...
    if (this.$route.query.id) {
      this.$http.get(api_prefix + `advertisement/get/${this.$route.query.id}`).then((res) => {
        if (res.data.body) {
      ...
      this.$refs["saveForm"].validate((valid) => {
        if (valid) {
          ...
          if (this.submitArr.length == 0) {
            this.$message.error('投放范围不能为空');
            return;
          } else {
            this.saveForm.areaList = this.submitArr
            this.btnFlag = true;
            let apiUrl = this.id ? "/advertisement/update" : "/advertisement/save"
