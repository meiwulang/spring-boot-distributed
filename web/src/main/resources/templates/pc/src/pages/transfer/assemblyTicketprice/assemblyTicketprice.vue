    ...
    <div class="jdy-content-inner-trip">
      <el-row class="jdy-content-inner-trip border p15 pl25">
        ...
        <el-col :span="8">
          ...
          <el-button type="primary" @click="onSubmitGo('assemblySchedule')" class="fright mr10 mt10">下一步</el-button>
          <!-- <el-button class="fright mt10" @click="goback('assemblyLine')">上一步</el-button> -->
        </el-col>
      </el-row>
        ...
        <div class="jdy-search jdy-search-bg2 clearfix ">
          <div class="p10 clearfix c5e">
            ...
            <div class="fright">
              ...
              <el-button type="primary" @click="searchTableData" class="mt10">搜索</el-button>
              ...
              <template scope="scope">
                <el-table :data="scope.row.tickList" border class="childBill inner-table" style="border:none" highlight-current-row @current-change="handleRowChange">
                  ...
                  <!-- <el-table-column label="状态" min-width="80">
                   <template scope="scope">
                      <el-switch v-model="scope.row.tStatus" disabled on-color="#71dc1c" :on-value="0" :off-value="1" on-text="" off-text="" @change="setStatus" @click.native="getRowId(scope.row.id)">
                      </el-switch>
                    </template>
                  </el-table-column> -->
...
export default {
  ...
  mounted(){
    ...
    // this.searchParams.tCompanyId = logindata.uCompanyId;
    this.getTableData();
  },
  methods: {
    ...
    editFare(_id,_priceid,_tTicketType){
      if(_tTicketType==0){
          this.$router.push({name:'assemblyAddfare',query:{id: _id,priceId: _priceid,type:1}})
      }else{
    ...
    addProduct(value) {
      ...
      if (value == 1) {//1.票价添加 2.添加套票  3.批量添加
        //  location.href = api_prefix+'/transfer.html#/ticketprice/addfare';
         this.$router.push({name:'assemblyAddfare',query:{id: this.$route.query.id}})
      }
      if (value == 2) {
