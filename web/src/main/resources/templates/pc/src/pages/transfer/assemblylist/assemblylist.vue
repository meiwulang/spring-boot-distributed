  ...
  <div class="jdy-content jdy-transfer fleft jl-information jl-List">
    <el-row class="jdy-content-inner">
      ...
      <div class="jdy-tab">
        ...
        <a href="javascript:;" title="" @click="jdytab(3)" :class="{ active: jdytabActive==3 }">已入库
          <span></span>
        </a>
        <a href="javascript:;" title="" @click="jdytab(2)" :class="{ active: jdytabActive==2 }">待入库
          <span></span>
        </a>
        <a href="javascript:;" title="" @click="jdytab(0)" :class="{ active: jdytabActive==0 }">已发布
          <span></span>
        </a>
        <!-- <el-button type="default" class="fright mt10 mr10">
                        <a id="to-line-page" href="#/line">添加产品</a>
                      </el-button> -->
        <!-- <el-dropdown split-button type="defult" class="fr mr10 mt10"> -->
          <!-- <a id="to-line-page" href="#/assemblylist/assemblyLine">添加集结产品</a> -->
          <!-- <el-dropdown-menu slot="dropdown">
            <el-dropdown-item>
              <span @click="gotoEditPage(7)">复制产品</span>
            </el-dropdown-item>
          </el-dropdown-menu> -->
        <!-- </el-dropdown> -->
        <!-- <el-button type="default" class="fright mt10" @click="goAssemblyLine">添加集结产品</el-button> -->
        <el-button type="default" class="fright mt10" @click="updatePage" v-loading.fullscreen.lock="fullscreenLoading">刷新</el-button>
        <el-button type="default" class="fright mt10 mr10" @click="alertJournal">日志</el-button>

      </div>
      <!--jdy-tab end-->
      <!--jdy-search begin-->
      <div class="jdy-search mt20">
        <el-select v-model="searchParams.pType" placeholder="请选择类型" class="jdy-search-edit hasbg mr10">
          <el-option v-for="(key,value) in type" :label="key" :value="value" :key="value">
          </el-option>
        </el-select>
        <el-select v-model="searchParams.companyId" placeholder="请选择分公司" v-show="companyIdFlag" clearable @change="getTableData">
            <el-option v-for="item in BuyerCompanyIdArr" :key="item.id" :label="item.cName" :value="item.id"></el-option>
        </el-select>
        <!-- <el-select v-model="searchParams.companyId" filterable placeholder="请选择单位" v-show='companyType==2' clearable remote :remote-method="searchCompanies" class="hasbg mr10 jl-companyInput" style="width:160px">
          <el-option v-for="item in companiesOptions" :label="item.cName" :value="item.id" :key="item.id">
          </el-option>
        </el-select> -->

        <div class="inlineblock mr10" style="width:305px;">
          <el-input placeholder="产品名称、拼音码" v-model="searchParams.pName" class="">
            <el-select v-model="searchParams.pBrand" filterable slot="prepend" placeholder="请选择品牌" v-if='companyType!=2' clearable  remote :remote-method="searchBrands" class="hasbg" style="width:120px">
              <el-option v-for="item in brandList" :label="item.bName" :value="item.id" :key="item.id">
              </el-option>
            </el-select>
          </el-input>
        </div>
      ...
      <!--table begin-->
      <div class="jdy-table">
        <el-table :data="tableData" border class="mt20 all" @row-click="rowclickHandler" highlight-current-row>
          <el-table-column type="expand" class="list-more">
            <template scope="scope">
              <span v-if="scope.row.trip.length>0">
                <span v-for="item in scope.row.trip" v-if="item.tripStartCalendar" :key="item.id" style="display:block;height:35px;line-height:35px;">{{item.tripName}} 行程时间:{{item.tripStartCalendar}}--{{item.tripEndCalendar}}
                  <div class="fr" style="padding-left:30px;">
                    <el-button type="default" size="mini" v-if="item.tripStartCalendar">
                      <router-link :to="{ path: '/list/supplement', query: {id: scope.row.id,isAdd: false,pDays:scope.row.pDays,tcStartDay:item.tripStartCalendar,tcEndDay:item.tripEndCalendar}}">编辑</router-link>
                    </el-button>
                    <el-button type="default" style="color:red" size="mini" v-if="item.tripStartCalendar" @click="deleteTrips(scope.row.id,item.tripStartCalendar,item.tripEndCalendar)">删除</el-button>
                  </div>
                </span>
              </span>
              <span v-else>暂无补充行程</span>
            </template>
          </el-table-column>
          <el-table-column label="产品编号" width="100" prop="pNo">
          </el-table-column>
          <el-table-column label="产品名称" min-width="200">
            <template scope="scope">
              <div style="text-align:left;padding:10px 0 10px 30px;">
                <a href="javascript:;"><span @click="gotoH5(scope.row.id)" style="color:#20a0ff">{{ scope.row.pName }}</span></a>
          ...
          <el-table-column label="区域产品所属分公司" width="150" prop="">
            <template scope="scope">
              {{ scope.row.companyName }}
            </template>
          </el-table-column>
          <el-table-column label="电子合同" min-width="100">
            <template scope="scope">
          ...
          <el-table-column label="客服" min-width="100">
            <template scope="scope">
              {{ scope.row.assembleLinkUserName }}
              <el-button type="default" size="mini" class="red fr" @click="showAssembleLinkUser">编辑</el-button>
            </template>
          </el-table-column>
          <el-table-column label="产品类型" width="100">
            <template scope="scope" >
              <span v-for="(key,value) in type" :key="key">
          ...
          <el-table-column label="操作" width="190">
            <template scope="scope">
              <el-select v-model="editType[scope.row.id]" @change="gotoEditPage" size="mini" class="goto-update" placeholder="编辑" value="">
                ...
                <el-option key="3" value="3" label="票价信息"></el-option>
                <el-option key="4" value="4" label="班期信息"></el-option>
                <!-- <el-option key="5" value="5" label="添加补充行程"></el-option> -->
                <!-- <el-option key="6" value="6" label="关键词设置"></el-option> -->
                <el-option key="9" value="9" label="合同模板"></el-option>
                <el-option key="8" value="8" label="添加成本"></el-option>
              </el-select>
              ...
              <el-button type="default" v-if="scope.row.pStatus!=3&&scope.row.pStatus!=0" @click="deleteProduct(scope.row.id)" size="mini" class="red mleft0">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <!--修改客服 弹窗  begin-->
      <jdy-alert title="编辑客服" @closeAlert="closeAlert" v-if="userFlag" class="alertCityList showtfcity" style="width:400px;">
        <el-form class="assembleForm" :model="assembleForm" :rules="rules" ref="assembleForm" style="margin-top:20px;padding:0 15px;">
          <el-row>
            <el-col :span="24">
              <el-form-item prop="signDate">
                <el-select v-model="assembleForm.assembleLinkUser" class="all" :filterable="true" :remote-method="searchContacts" clearable remote @change="changeAssembleLinkUser">
        ...
        <div class="alertfoot1 clearfix">
          <el-button class="fright mt10 mr10" @click="saveUser" type="primary" :disabled="btnFlag">提交</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        lSort: [{ type: "number", message: "排序必须为数字值" }]
      },
  ...
  methods: {
    ...
    //
    goAssemblyLine(){
      this.$router.push({ name: 'assemblyLine'});
    },
    //删除关键词关联关系
    deleteRelation(item) {
      this.$http
        .post(
          api_prefix +
            "/productKey/deleteProductKeyRelation?keyId=" +
            item.id +
            "&productId=" +
            this.pid
        )
    ...
    saveAddTag() {
      this.$refs.newKeywordForm.validate(valid => {
        if (valid) {
          this.$http
            .post(api_prefix + "productKey/saveNewKey", this.newKeywordForm)
            .then(
              response => {
                if (response.data.code == 0) {
                  this.$message({
    ...
    getAllProductsKeywords() {
      let _self = this;
      this.$http
        .post(api_prefix + "productKey/selectKeyListByCompanyId", {
    ...
    getProductsKeywords() {
      ...
      this.$http
        .post(
          api_prefix +
            "/productKey/selectKeyListByProductId?productId=" +
            this.pid
        )
        .then(
          response => {
            ...
            response.data.body.forEach(function(value) {
              ...
              _self.setkeyTagsNumber =
                response.data.body.length > 0 ? response.data.body.length : 0;
            });
          },
          response => {
            console.log("获取日志出错了");
    ...
    gotoEditPage(val) {
      ...
      if (val == 1) {
        location.href = "/transfer.html#/assemblylist/assemblyLine?id=" + this.pid;
        return;
      }
      if (val == 3) {
        location.href = "/transfer.html#/assemblylist/assemblyTicketprice?id=" + this.pid;
        return;
      }
      if (val == 4) {
        location.href = "/transfer.html#/assemblylist/assemblySchedule?id=" + this.pid;
        return;
      }
      if (val == 8) {
        location.href = "/transfer.html#/assemblylist/addCostList?id=" + this.pid+"&c_id="+this.companyId;
        return;
      }
      if (val == 9) {
        this.getContract();
      ...
      if (val == 7) {
        location.href = "#/list/assemblylist?copyId=" + this.pid;
      }
    },
    // 合同模板-查询
    getContract() {
      this.$http
        .get(
          api_prefix +
            "front/order/m/electronicContract/prodTempList/" +
            this.pid
        )
        .then(
          response => {
    ...
    saveContract() {
      ...
      this.$http
        .post(
          api_prefix + "front/order/m/electronicContract/bindProAndTmp",
          param
        )
        .then(
          response => {
    ...
    deleteProduct(id) {
      this.$confirm("是否要删除该产品？", "温馨提示", {
        ...
        .then(() => {
          // console.log("id",id)
          this.$http.post(api_prefix + "product/delete/" + id).then(
            response => {
              if (response.data.code == 0) {
    ...
    queryUserList(companyId) {
      this.$http.post(api_prefix + '/user/list', { "uCompanyId": companyId, "uRealName": this.contactsName,"uStype":0 ,"pageSize":10000}).then(rsp => {
        if (rsp.body.code == 0) {
    ...
    saveUser(){
      this.btnFlag = true;
      this.$http.post(api_prefix + "productAssembleCompany/modifyAssembleLink",{
        assembleLinkUser: this.assembleForm.assembleLinkUser,
        productId: this.pid,
    ...
    changeAssembleLinkUser(id){//选择客服后函数
      this.assembleForm.assembleLinkUser = id;
    },
    closeAlert() {
      //关闭弹窗统一方法
      this.userFlag = false;
      this.assembleForm.assembleLinkUser = '';
      this.peopleList = [];
      this.contactsName = '';
      this.queryUserList(JSON.parse(sessionStorage.loginData).uCompanyId)
      $(".alertbgg").remove();
    ...
    deleteTrips(pid, tripStartCalendar, tripEndCalendar) {
      ...
      this.$confirm("你确定要删除该行程？", "温馨提示", {
        ...
        this.$http
          .post(api_prefix + "/trip/batchDelete", this.deleteTripsParams)
          .then(
            response => {
              if (response.data.code == 0) {
                this.$message({
    ...
    changeConfirm(val) {
      ...
      this.$http.post(api_prefix + "/product/update", val).then(
        response => {
          if (response.data.code != 0) {
            this.$alert(response.data.message);
          }
    ...
    searchBrands(val) {
      ...
      this.brandList = null;
      this.$http.post(api_prefix + "/brand/list", this.brandSearchParams).then(
        response => {
    ...
    addProductKey() {
      this.$http
        .post(
          api_prefix +
            "productKey/saveProductKeyRelation?productId=" +
            1 +
            "&keyId=" +
            1
        )
    ...
    effect(companyId, id, updateUser) {
      this.$confirm("你确定要将该产品入库？", "温馨提示", {
        ...
        this.$http
          .post(api_prefix + "product/effect", {
            companyId: companyId,
            id: id,
            puserId: updateUser
          })
    ...
    uneffect(companyId, id, updateUser) {
      this.$confirm("你确定要将该产品移出？", "温馨提示", {
        ...
        this.$http
          .post(api_prefix + "product/uneffect", {
            companyId: companyId,
            id: id,
            puserId: updateUser
          })
    ...
    getTableData() {
      ...
      this.$http.post(api_prefix + "aggr/local-productlist", params).then(
        response => {
          let data = response.data.body;
    ...
    saveSetkey(a, b, c) {
      ...
      if (this.productsKeywords.length > 4) {
        this.$alert("最多可以添加4个关键词");
        return;
      }
      //if(this.currentSelectKey==null){this.$alert('请选择词条');return ;}
      if (this.productsKeywords.length < 1) {
        this.$alert("请选择词条");
        return;
      }
      let keyIds = [];
      ...
      //保存关联关系
      this.$http
        .post(api_prefix + "/productKey/batchSaveProductKeyRelation", {
          productId: this.pid,
          keyIds: keyIds
        })
        .then(
    ...
    changeViewBtn() {
      ...
      switch (this.newKeywordForm.kColor) {
        case 0:
          $(".alertTagTop .jl-TagViewBtn").removeClass(
            "bkone bktwo bkthree bkfour bkfive bksix bkseven bkeight"
          );
