            ...
            <el-form ref="form" label-width="100px" :model="fromData" :rules="rules">
              <el-col :span="12">
                <el-form-item label="用户名:" prop="uAccount">
                  <el-input v-model="fromData.uAccount" :disabled="true"></el-input>
                </el-form-item>
                ...
                <el-form-item label="选择部门:">
                  <el-select v-model="fromData.uDepartmentId" @change="userAll" :filterable="true" :remote-method="searchDepartment" clearable remote placeholder="全部部门" class="gz-Block">
                    <el-option v-for="item in userOptions" :key="item.dCode" :label="item.dName" :value="item.id">
                      <div @click="getCode(item.dCode)">{{item.dName}}</div>
                    </el-option>
                  </el-select>
                </el-form-item>
              ...
              <el-col :span="12">
                ...
                <el-form-item label="代理人角色:">
                  <el-radio-group v-model="fromData.uDtype" prop="uDtype">
                    <template scope="scope">
                      <el-radio :label="1">负责人</el-radio>
                      <el-radio :label="0">普通代理人</el-radio>
                    </template>
                  </el-radio-group>
                </el-form-item>
              ...
              <el-col :span="12">
                <!-- <el-form-item label="性别:" prop="region">
                  <el-select v-model="fromData.uSex" @change="uSexFun" class="gz-Block">
                    <el-option :label="sex.label" :value="sex.value" v-for="sex in sexTypes"></el-option>
                  </el-select>
                </el-form-item>
...
export default {
  ...
  methods: {
    getCode(val){
      console.log(val,'val')
      this.fromData.uDepartmentCode = val;
    },
    //重置密码
    resetPassword() {
    ...
    defaultFromData() {
      ...
      this.$http.get(api_prefix + 'user/selectDistUserInfo/' + index, ).then(response => {
        if (response.data.code == 0) {
          ...
          // this.uDepartmentId = response.body.body.uDepartmentId
          console.log(response.body.body.uDepartmentId, 'response.body.body.uDepartmentId')
          setTimeout(() => {
            ...
            if(response.body.body.uDepartmentId == -1){
    ...
    //选择部门
    userAll(value) {
      // this.uDepartmentId = value.id
      this.fromData.uDepartmentId = value
      console.log(this.uDepartmentId,'this.uDepartmentId')
    },
    //角色权限
    uTpyeFun(value) {
    ...
    //返回
    backUser() {
      this.$router.push({ path: "/agentUser" })
    },
    //保存
    saveFun(formName) {
      // this.saveParameter()
      ...
      if (this.$route.query.isAdd == true) {
        ...
        var url = api_prefix + '/user/updateDistUser'
      }
          ...
          this.$http.post(url, this.fromData).then(response => {
            ...
            if (response.body.code == 0) {
              ...
              setTimeout(() => {
                ...
                this.$router.push({ name: 'agentUser' })
              }, 1000);
            } else {
              this.$alert(response.data.message, '温馨提示', {
                confirmButtonText: '确定',
