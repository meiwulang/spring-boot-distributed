    ...
    <el-row class="jdy-content-inner">
      ...
      <div class="jdy-tab">
        ...
        <el-dropdown split-button type="defult" class="fr mr10 mt10">
          <a id="to-line-page" href="#/list/line">添加产品</a>
        ...
        <el-button type="default" class="fright mt10 mr10" @click="updatePage" v-loading.fullscreen.lock="fullscreenLoading">刷新</el-button>
      ...
      <div class="jdy-search mt20">
        ...
        <el-select v-model="searchParams.ascription" class="jdy-search-edit hasbg mr10">
          <el-option  label="本地" :value="0" :key="0">
          </el-option>
          <el-option  label="区域" :value="1" :key="1">
          </el-option>
        </el-select>
        <el-select v-model="searchParams.pGroup" class="jdy-search-edit hasbg mr10" placeholder="请选择大区" clearable>
          <el-option :key="item.id" v-for="item in groupList" :label="item.dName" :value="item.id">
          </el-option>
        </el-select>
        <el-select v-model="searchParams.pDestination" class="jdy-search-edit hasbg mr10" placeholder="请选择目的地" clearable>
      ...
      <div class="jdy-table">
        <el-table :data="tableData" border class="mt20 all" @row-click="rowclickHandler" highlight-current-row>
          ...
          <el-table-column label="产品归属" min-width="60">
            <template scope="scope">
              <span>{{ scope.row.ascription==0?"本地":"区域"}}</span>
            </template>
          </el-table-column>
          <el-table-column label="产品所属分公司" min-width="100">
            <template scope="scope">
...
export default {
  ...
  methods: {
    ...
    gotoEditPage(val) {
      ...
      if (val == 1) { location.href = "/transfer.html#/list/line?id=" + this.pid; return; }
      if (val == 2) { location.href = "/transfer.html#/list/trip?id=" + this.pid; return; }
      if (val == 3) { location.href = "/transfer.html#/list/ticketprice?id=" + this.pid; return; }
      if (val == 4) { location.href = "/transfer.html#/list/schedule?id=" + this.pid; return; }
      if (val == 5) { if (this.tripDays < 1) { this.$alert("请先添加默认行程"); return } location.href = "/transfer.html#/list/supplement?id=" + this.pid + '&pDays=' + this.tripDays + '&isAdd=' + true; return; }
      if (val == 6) { this.alertSetkeyFlag = true; }
      if (val == 8) { location.href = "/transfer.html#/list/addCostList?id=" + this.pid+"&c_id="+this.c_id; return; }
      if (val == 9){
        this.getContract()
      }
      if (!this.pid) {
      ...
      if (val == 7) { location.href = "#/list/line?copyId=" + this.pid }
    },
    // 合同模板-查询
    getContract(){
