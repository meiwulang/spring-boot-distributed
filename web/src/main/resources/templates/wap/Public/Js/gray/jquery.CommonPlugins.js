...
(function($){
    ...
    $.fn.tabs = function (opt,obj) {
        ...
        if(t_move_class){
            var t_move_class = opt.t_move_class,
                class_len = t_move_class.length;
        }
        ...
        function Event_fn(t) {
            ...
            if (t_move_add) {
                for (var i = 0; i < len; i++) {
                    $(t_move_add[i][0]).removeClass(t_move_add[i][1]);
                    $(t_move_add[i][0]).eq(n).addClass(t_move_add[i][1]);
                }
            }
            if (t_move_class) {
...
/**
    ...
    $.fn.scrollAnchor= function(opt){
        ...
        function days_fn(wh){
            ...
            setTimeout(function(){
                cont_left.find('.cl-l').removeClass('cl-hover');
                cont_left.find('.cl-l').eq(days_n).addClass('cl-hover');
            },200);

        }

        sa_id.find('.item-l').click(function(){ item_fn(this,opt.GotoCls);});
        cl_l.click(function(){ item_fn(this,'.xcsm-box .days-icon'); });
    ...
    $.fn.insertAtCaret=function(myValue){
        ...
        if (document.selection) {
            ...
            if ($t.selectionStart || $t.selectionStart == '0') {
                ...
                var endPos = $t.value.indexOf(']') == -1 ? startPos : $t.selectionEnd;
