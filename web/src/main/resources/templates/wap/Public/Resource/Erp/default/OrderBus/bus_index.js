...
Ext.onReady(function(){
    var store = SUNLINE.GroupingStore($__app__+'/OrderBus/order_bus_json',[],'',true,'',200);

    //时间格式
    function date_format(v,c,r){
        var fb=font_bold(v,r);
        if(fb['status']==true)return fb['msg'];
        return int2date(v);
    }
    //金额格式
    ...
    function number_site(v,c,r){
        var fb=font_bold(v,r);
        if(fb['status']==true)return '';
        var small='';
        if(r.get('s_ticket_type')=='儿童票')small='/孩';
        if(r.get('s_ticket_type')=='全陪票')small='/全陪';
        return '<div class="trans_list_cls"><font style="color:green"><b>'+ r.get('ob_bus_number')+'</b></font>'+v+small+'</div>';
    }
    function num_fn(v,c,r){
        var fb=font_bold(v,r);
        if(fb['status']==true)return fb['msg'];
        return v;
    }

    function money_fn(v,c,r){
        var fb=font_bold(v,r,'money');
        if(fb['status']==true)return fb['msg'];
        return money(v);
    }

    function font_bold(v,r,f){
        if(r.get('ob_start_date')!='总合计'&&r.get('ob_start_date')!='当页合计')return v;
        if(f=='money')v=money(v);
        return {status:true,msg:'<b>'+v+'</b>'};
    }

    var cm = [
        {header:'ID',dataIndex:'id',width:60,hidden:true},
        {header:'票务日期',dataIndex:'ob_start_date',align:'right',width:90,renderer:date_format},
        /*{header:'票务公司',dataIndex:'ob_out_site',width:150,renderer:torg_name_fn},*/
        {header:'航班次信息',dataIndex:'ob_number',width:240,renderer:number_site},
        {header:'下铺',dataIndex:'ob_one',width:80,align:'center',renderer:num_fn},
        {header:'中铺',dataIndex:'ob_two',width:80,align:'center',renderer:num_fn},
        {header:'上铺',dataIndex:'ob_three',width:80,align:'center',renderer:num_fn},
        {header:'张数',dataIndex:'ob_num',width:80,align:'center',renderer:num_fn},
        {header:'手续费',dataIndex:'ob_price',width:90,align:'right',renderer:money_fn},
        {header:'金额',dataIndex:'ob_money',align:'right',width:120,renderer:money_fn},
        {header:'用途',dataIndex:'p_name',align:'left',width:180}
    ];
    ...
    var order_type=SUNLINE.LocalComob({
        id:'s_o_type',
        fields:['s_o_type'],
        data:[['全部用票'],['散客用票'],['团队用票']],
        config:{
    ...
    function download_excel(){
        Ext.MessageBox.confirm('友情提示','你确定要导出当前条件下的数据吗?',function(y){
            ...
            url=url.join('/');
            window.location = $__app__+'/OrderBus/order_bus_json_excel/'+url;
        });
    }
