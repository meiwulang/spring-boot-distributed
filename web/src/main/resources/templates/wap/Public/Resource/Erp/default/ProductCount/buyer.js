...
Ext.onReady(function () {
    ...
    var cm = [
        ...
        {header: '分销商ID', dataIndex: 'org_id', width: 100, align: 'center', hidden: true},
        {header: '分销商名称', dataIndex: 'org_name', width: 200, align: 'left'},
        ...
        {
            header: "供应商数 <i class='fa fa-question-circle'></i>",
            dataIndex: 'org_count',
            align: 'right',
            width: 80,
            renderer: goSupplyList,
            tooltip: '点击供应商数可跳转到该单位下相应的供应商汇总报表'
        },
        {
    ...
    //store
    var url = $__app__ + '/ProductCount/buyerDataList';
    ...
    if(cur_province || cur_city || cur_area){
        ...
        tbar: [
            {
                ...
                items:[{
                    ...
                    {xtype:'tbtext', text:''},
                    {icon: $app_public_images_path + 'b_help.png',tooltip: '查看帮助 ', cls: 'x-btn-icon',handler:function(){help('分销商汇总报表')}},
                ],
            }
        ],
    ...
    function doExcel() {
        ...
        //把搜索条件拼接到url
        var params = 'sb=2&time_type=' + search_params.time_type
            + '&org_id=' + org_id
    ...
    function showWin(v, m){
        ...
        v = !org_id ? v : "<a href='javascript:;' onclick='getOrderList(\"o_sorg_id\", "+org_id+", \""+org_name+"\");' style='color:blue;text-decoration：none;'>"+v+"</a>";
        return v;
    }

    //弹窗显示--订单列表
    window.getOrderList = function(org_type, sorg_id, org_name){
    ...
    //获取供应商数链接
    function goSupplyList(v, m){
        console.log(org_id);
        var org_id  = m.record.data.org_id;
        var org_name = m.record.data.org_name;

        v = (!org_id || gone) ? v : "<a href='javascript:;' onclick='getSupplyList("+org_id+", \""+org_name+"\");' style='color:blue;text-decoration：none;'>"+v+"</a>";
        return v;
    }

    //点击供应商数跳转到供应商汇总
    window.getSupplyList = function(org_id, org_name){
        var start_time = Ext.Date.format(Ext.getCmp('start_time').getValue(), 'Y-m-d');
        var end_time = Ext.Date.format(Ext.getCmp('end_time').getValue(), 'Y-m-d');
        var time_type = Ext.getCmp('time_type').getValue();

        var open_title = '【销】'+org_name+'的';
        var open_id    = "SbCountBuyer_"+Math.round(Math.random()*1000);

        var url = $__app__ + '/ProductCount/supplier?org_id='+org_id;
        ...
        parent.OpenTab(open_title+'供应商汇总', open_id, '', url, 1);
    }
    ...
    //点击跳向产品汇总报表
    window.getProductList = function(sorg_id, org_name){
        var start_time = Ext.Date.format(Ext.getCmp('start_time').getValue(), 'Y-m-d');
        var end_time = Ext.Date.format(Ext.getCmp('end_time').getValue(), 'Y-m-d');
        var time_type = Ext.getCmp('time_type').getValue();
        ...
        var url = $__app__ + '/ProductCount/index?org_id='+sorg_id;
        url += "&t_type=2";
        url += "&t_org_id=" + org_id;
        url += "&time_type=" + encodeURI(time_type);
        url += "&start_time=" + encodeURI(start_time);
