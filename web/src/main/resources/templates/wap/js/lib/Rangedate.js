...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    var dateBegin = new Date(year, month+1, 1);
    var w = dateBegin.getDay();
    // 计算应该开始的日期
    dateBegin.setDate(1 - w);

    // 当月最后一天
    var lastDayOfMonth = new Date(year, month + 2, 0);
    var dateEnd = new Date(year, month + 2, 0);
    w = dateEnd.getDay();
    // 计算应该结束的日期
	...
	if(isTaTheme){
		...
		if(this.mOpts.magicSelect){
			...
			yh.push('</ul></div></div>');
			mh.push('</ul></div></div>');			
			$(cap).append(yh.join('') +'<span class="joinLine"> 年 </span>'+mh.join('')+1+'<span class="joinLine"> 月 </span>');

		}
		else{
			$(cap).append(year + '年' + (month + 2) + '月');
		}
