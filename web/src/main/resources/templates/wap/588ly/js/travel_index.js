...
function getBannerData() {
    code = _GetCityCode();
    // console.log(code);
    var getBannerUrl = '/b2b/adver/index_adver?city_code=' + code + '&&type=无锡-首页广告';
    ajaxRequest(getBannerUrl, 'GET', '', function (ret) {
        if (ret.data && ret.code == 200) {
            var dataInter = ret.data;
            var interText = doT.template($('#tpl-banner').text());
            $('.banner').html(interText(dataInter));
            new Mango();
        } else {
...
function getNewslistsData() {
    code = _GetCityCode();
    var getnewslistsUrl = '/b2b/news/lists?limit=5&city_code=' + code
