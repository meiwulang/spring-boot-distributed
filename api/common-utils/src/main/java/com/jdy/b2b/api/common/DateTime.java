...
public class DateTime {
     ...
     * @return
     */
    public static Date addTimeByDays(Date dateTime, int days) {
        //Date转换到calendar
        Calendar cal = Calendar.getInstance();
        cal.setTime(dateTime);
        //在calendar基础上新增一小时
     ...
     * @return
     */
    public static Date str2Date(String date, String fromat) {
        Date d = null;
        try {
    ...
    public static String date2qua(String endtime) {
        ...
        if (StringUtils.isNotNullOrEmptyStr(endtime) && endtime.length() == 10) {
     ...
     * @return
     */
    public static Map<String, String> getStartDateAndEndDate(String type,Date date) {
        String currDate = currentDateTime(DateFormat1);
        ...
        if ("currmonth".equals(type)) {
            ...
            if(!org.springframework.util.StringUtils.isEmpty(date)){
                cal.setTime(date);
                end_date = date2Str(date,DateFormat1);
            }else{
                end_date = currDate;
            }
    ...
    public static Date formatDate(String startDate) {
        Date date = null;
        if (StringUtils.isNotNullOrEmptyStr(startDate)) {
            try {
                date = new Date(Long.valueOf(startDate));
            } catch (Exception e) {
                date = null;
            }
        }
    ...
    public static Date formatDate(String startDate, String dateFormat1) {
        Date date = null;
        if (StringUtils.isNotNullOrEmptyStr(startDate)) {
            try {
                date = new SimpleDateFormat(dateFormat1).parse(startDate);
            } catch (Exception e) {
                date = null;
            }
        }
    ...
    public static List<Date> findDates(Date begin, Date end) {
        ...
        if (begin.getTime() > end.getTime()) return date;
    ...
    /**
     * 转换时间  年月日时分秒 为  年月日 列如: 2017-03-29 16:17:35  --> 2017-03-29 00:00:00
     *
     * @param date
     * @return
     */
     ...
     * @return
     */
    public static boolean dateEquels(Date d1, Date d2, String format) {
        if (d1 == null || d2 == null || StringUtils.isNullOrEmptyStr(format)) return false;
        return DateTime.date2Str(d1, format).equals(DateTime.date2Str(d2, format));
    }
